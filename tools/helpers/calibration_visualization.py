...
import tools.helpers.metrics as metrics
...
class ConfidenceHistogram(metrics.MaxProbCELoss):
    ...
    def plot(self, output, labels, n_bins=15, logits=True, title=None):
        ...
        if acc > conf:
            plt.text(acc + 0.03, 0.7, 'Accuracy', rotation=90, fontsize=11)
            plt.text(conf - 0.07, 0.7, 'Avg. Confidence', rotation=90, fontsize=11)
        else:
            plt.text(acc - 0.07, 0.7, 'Accuracy', rotation=90, fontsize=11)
            plt.text(conf + 0.03, 0.7, 'Avg. Confidence', rotation=90, fontsize=11)
...
class ReliabilityDiagram(metrics.MaxProbCELoss):
    ...
    def plot(self, output, labels, n_bins=15, logits=True, title=None):
        ...
        plt.bar(x, error, bottom=np.minimum(self.bin_acc, mid), color='mistyrose', alpha=0.5, width=delta, align='edge',
        ...
        if title is not None:
            plt.title(title, fontsize=16)
        ece = np.dot(self.bin_prop, self.bin_score)
        plt.title(f'ECE={ece:.3f}')
        plt.tight_layout()
