...
class FlexMatch(AlgorithmBase):
    ...
    def train_step(self, x_lb, y_lb, idx_ulb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            def hessian_wrapper(*params):
                logits = _stateless.functional_call(
                    self.model, {n: p for n, p in zip(names, params)}, x_lb)
                return ce_loss(logits, y_lb, reduction='mean')
                # return out.square().sum()
            temp = hessian(hessian_wrapper, tuple(self.model.parameters()))
            print(temp)
            # self.model.module.debug_mode = False
            ...
            self.model.module.debug_mode = True
            names = list(n for n, _ in self.model.named_parameters())
            temp = jacobian(lambda *params: _stateless.functional_call(self.model, {n: p for n, p in zip(names, params)}, x_lb),
            ...
            # jacobian(lambda *params: _stateless.functional_call(model, {n: p for n, p in zip(names, params)}, x_lb),
